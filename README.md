...
| 目标 | 结论 |
| --- | --- |
| 较新 Chromium 内核 | ✅ 能做到，但“较新”是相对的：覆盖 iOS 10 只能用 **2019 年前后的 Chromium（约 M75）**，它比 iOS 10 自带 Safari(2016) 新很多、能跑现代网页；**不是 2024 的最新版**。 |
| 支持 iOS 10 以上所有越狱设备 | ⚠️ **64 位设备全覆盖**（iOS 10/11/12）。**32 位 iPhone 5 / 5c 做不到**——Chromium 自 2016 年起就不再出 32 位 iOS 包，物理上不可能。 |
...
**核心取舍**：现代 Chromium（M114+）最低只支持 iOS 13/15+ 且只有 arm64；要与 iOS 10 兼容，必须用老一代 Chromium。
想要更新的内核 → 把 `chromium_ref` 调到 M83~M90（仅 iOS 11/12+），详见 §6。
...
> ⚠️ 无论哪种，**都不能用 GitHub 官方托管的 macOS runner**：它只有 ~14GB 磁盘/内存、单任务 6 小时上限，
> Chromium 编译需要 30~50GB 磁盘 + 数小时，必失败。所以 Actions 方案用的是**你的 Mac 作为自托管 runner**。

---

## 2. 前置条件（你的 Mac）

- macOS（建议与所选 Chromium 同代：M75 → Xcode 10.1~11；越新越可能需要补丁，见 `patches/README.md`）
...
仓库 → **Actions → “Build Chromium IPA (legacy iOS 10+, arm64)” → Run workflow**。
可选填：
- `chromium_ref`：默认 `75.0.3770.142`（M75，支持 iOS 10 的一代附近）
- `deployment_target`：默认 `10.0`
...
# 或指定版本 / iOS 最低版本（例如想要更新内核、仅 iOS 11+）
./scripts/build_local.sh 83.0.4103.119 11.0
```
...
| 想做的事 | 怎么做 |
| --- | --- |
| 想要**更新的内核**（代价：放弃 iOS 10，仅 iOS 11/12+） | `chromium_ref` 改为 `83.0.4103.119`(M83) / `90.0.4430.212`(M90)，`deployment_target` 改 `11.0` 或 `12.0`。 |
| 坚持 iOS 10 但想尽量新 | 在 M75 附近微调（M74~M76），超过 M76 大多已要求 iOS 11。 |
...
- **iOS 10 真机启动闪退**：多为 API 不可用，需对应补丁或调 `deployment_target` 到 `11.0`。
